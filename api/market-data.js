...
function parseS(arr){return(arr||[]).filter(i=>i&&i.hts_kor_isnm).map(i=>({name:i.hts_kor_isnm,code:i.mksc_shrn_iscd||i.stck_shrn_iscd,price:+i.stck_prpr,change:+i.prdy_ctrt,amt:+(i.acml_tr_pbmn||0),amtFmt:fmt(+(i.acml_tr_pbmn||0)),vol:+(i.acml_vol||0),isLimit:+i.prdy_ctrt>=29}))}
...
module.exports=async(req,res)=>{
  ...
  try{
    ...
    const volAll=parseS(volJ.output);
    const seenVol=new Set(volAll.map(s=>s.code));
    parseS(volKQ).forEach(s=>{if(!seenVol.has(s.code)){volAll.push(s);seenVol.add(s.code);}});

    // 등락률 순위 (코스피 J + 코스닥 KQ) — 거래대금 무관
    ...
    let gainJ=[],gainKQ=[],gainErr="";
    try{
      const r=await get("/uapi/domestic-stock/v1/quotations/chgrate-rank","FHPST01700000",{...gp,FID_COND_MRKT_DIV_CODE:"J"},tk);
      gainJ=parseS(r.output||r.d?.output);
    }catch(e){gainErr+="J:"+e.message.slice(0,60)+" ";}
    await w(500);
    try{
      const r=await get("/uapi/domestic-stock/v1/quotations/chgrate-rank","FHPST01700000",{...gp,FID_COND_MRKT_DIV_CODE:"KQ"},tk);
      gainKQ=parseS(r.output||r.d?.output);
    }catch(e){gainErr+="KQ:"+e.message.slice(0,60);}

    const gainAll=[...gainJ];
    const seenGain=new Set(gainJ.map(s=>s.code));
    gainKQ.forEach(s=>{if(!seenGain.has(s.code)){gainAll.push(s);seenGain.add(s.code);}});
    const gainRanking=gainAll.filter(s=>s.change>0).sort((a,b)=>{
    ...
    res.status(200).json({
      ...
      total:volAll.length,
      debug:{kospi:(volJ.output||[]).length,kosdaq:volKQ.length,gainJ:gainJ.length,gainKQ:gainKQ.length,gainErr:gainErr||null}
    });
