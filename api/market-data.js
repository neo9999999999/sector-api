...
module.exports=async(req,res)=>{
    ...
    try{
          ...
          let kospiStocks=[],kosdaqStocks=[],kosdaqErr="";
          try{const vK=await get("/uapi/domestic-stock/v1/quotations/volume-rank","FHPST01710000",{...vp,FID_COND_MRKT_DIV_CODE:"J"},tk);kospiStocks=parse(vK.output);}catch(e){console.error("kospi:",e.message)}
          await w(1200);
          try{const vQ=await get("/uapi/domestic-stock/v1/quotations/volume-rank","FHPST01710000",{...vp,FID_COND_MRKT_DIV_CODE:"Q"},tk);kosdaqStocks=parse(vQ.output);if(!kosdaqStocks.length)kosdaqErr=JSON.stringify(vQ).slice(0,300);}catch(e){kosdaqErr=e.message}
