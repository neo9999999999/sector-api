...
const KOSDAQ=new Set(['한미반도체','리노공업','원익IPS','피에스케이','HPSP','이오테크닉스','테스','주성엔지니어링','에코프로','에코프로비엠','엘앤에프','천보','나노신소재','현대로템','퍼스텍','빅텍','HLB','알테오젠','한전기술','한전KPS','두산에너빌리티','에스엠','JYP Ent.','키움증권','크래프틴','한화에어로스페이스','LIG넥스원']);
const ETF=/^(KODEX|TIGER|KBSTAR|ARIRANG|KOSEF|HANARO|PLUS|RISE|ACE|SOL|WON|TIMEFOLIO|SMART|KB스타|NH|마이다스)/;
...
module.exports=async(req,res)=>{
  ...
  try{
    ...
    // 등락률 순위 — ranking/fluctuation 경로 (검증됨)
    ...
    res.status(200).json({
      ...
      topRising:rising,gainRanking,
      limitUp:volAll.filter(s=>s.isLimit),total:volAll.length,
      debug:{kospi:volAll.filter(s=>s.market!=="Q").length,kosdaq:volAll.filter(s=>s.market==="Q").length,gainApiOk,gainLen:gainRanking.length,gainErr:gainErr||null}
    });
