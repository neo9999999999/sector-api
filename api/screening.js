...
// 등락률 상위 페이지네이션 (최대 pages*30개)
async function getGainPages(tk,mktCode,mktName,pages){
  const all=[];
  const base={FID_INPUT_ISCD:"0000",FID_RANK_SORT_CLS_CODE:"0",FID_COND_SCR_DIV_CODE:"20170",FID_DIV_CLS_CODE:"0",FID_BLNG_CLS_CODE:"0",FID_TRGT_CLS_CODE:"111111111",FID_TRGT_EXLS_CLS_CODE:"000000",FID_INPUT_PRICE_1:"",FID_INPUT_PRICE_2:"",FID_VOL_CNT:"",FID_INPUT_DATE_1:""};
  for(let p=0;p<pages;p++){
    try{
      const qs=new URLSearchParams({...base,FID_COND_MRKT_DIV_CODE:mktCode}).toString();
      const h={authorization:"Bearer "+tk,appkey:AK,appsecret:AS,tr_id:"FHPST01700000",custtype:"P"};
      if(p>0)h.tr_cont="N";
      const r=await req("GET","/uapi/domestic-stock/v1/ranking/fluctuation?"+qs,h);
      const items=(r.output||[]).map(s=>parseS(s,mktName));
...
// 거래대금 상위 페이지네이션 (코스피만)
async function getVolPages(tk,pages){
  const all=[];
  const base={FID_INPUT_ISCD:"0000",FID_RANK_SORT_CLS_CODE:"0",FID_COND_SCR_DIV_CODE:"20171",FID_DIV_CLS_CODE:"0",FID_BLNG_CLS_CODE:"0",FID_TRGT_CLS_CODE:"111111111",FID_TRGT_EXLS_CLS_CODE:"000000",FID_INPUT_PRICE_1:"",FID_INPUT_PRICE_2:"",FID_VOL_CNT:"",FID_INPUT_DATE_1:""};
  for(let p=0;p<pages;p++){
    try{
      const qs=new URLSearchParams({...base,FID_COND_MRKT_DIV_CODE:"J"}).toString();
      const h={authorization:"Bearer "+tk,appkey:AK,appsecret:AS,tr_id:"FHPST01710000",custtype:"P"};
      if(p>0)h.tr_cont="N";
      const r=await req("GET","/uapi/domestic-stock/v1/quotations/volume-rank?"+qs,h);
      const items=(r.output||[]).map(s=>parseS(s,"KOSPI"));
      if(items.length===0)break;
      all.push(...items);
      await w(300);
    }catch(e){break}
...
module.exports=async(req,res)=>{
  ...
  try{
    ...
    // 코스피 거래대금 200 (7페이지)
    const vol=await getVolPages(tk,7);
    // 코스피 등락률 200 (7페이지)
    const gainK=await getGainPages(tk,"J","KOSPI",7);
    // 코스닥 등락률 200 (7페이지)
    const gainQ=await getGainPages(tk,"Q","KOSDAQ",7);
    // 합치기+중복제거+필터
    const seen=new Set();const all=[];
    [...vol,...gainK,...gainQ].forEach(s=>{if(!s.code||seen.has(s.code))return;seen.add(s.code);if(s.change>=10&&s.change<29&&s.amount>=50)all.push(s)});
    // 투자자 데이터 (최대 40개)
