// api/append-signals.js — 브라우저 루프에서 청크별로 호출해서 서버 JSON에 append
...
// 수동으로 req.body JSON 파싱 (Vercel은 기본 파싱이지만 안전하게)
...
module.exports = async (q, res) => {
  ...
  try {
    ...
    // POST: chunk append + commit
    const body = await readBody(q);
    const newSignals = body.signals || [];
    const chunkMeta = body.chunk || {};  // {start, size, processed, from, to}
    ...
    existing.chunks.push({...chunkMeta, ts: new Date().toISOString(), new_signals: newSignals.length});
    existing.last_updated = new Date().toISOString();

    // 3. PUT
    const newContent = Buffer.from(JSON.stringify(existing, null, 1)).toString('base64');
    const putBody = JSON.stringify({
      message: 'append signals chunk start=' + (chunkMeta.start || '?') + ' (+' + newSignals.length + ')',
      content: newContent,
      branch: 'main',
      ...(sha ? {sha} : {})
    });
    ...
    if (putR.status !== 200 && putR.status !== 201) {
      return res.status(putR.status).json({ok:false, error:'PUT failed', body: putR.body.slice(0,300)});
    }
    const pd = JSON.parse(putR.body);
    return res.status(200).json({ok:true, committed:true, total: existing.signals.length, added: newSignals.length, chunks: existing.chunks.length, sha: pd.content.sha});
  } catch(e) {
