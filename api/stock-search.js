...
function naverSearch(name){return new Promise(function(y,n){var url="/ac?q="+encodeURIComponent(name)+"&q_enc=euc-kr&st=111&frm=stock&r_format=json&t_koreng=1&r_lt=111";var r=https.request({hostname:"ac.finance.naver.com",port:443,path:url,method:"GET"},function(s){var t="";s.on("data",function(c){t+=c});s.on("end",function(){try{y(JSON.parse(t))}catch(e){n(new Error(t.slice(0,300)))}})});r.on("error",n);r.end()})}
