  ...
  try{
    ...
    for(const s of STOCKS){
      try{
        ...
        }else{results[s.code]={name:s.name,sector:s.sector,error:"empty",msg:r.msg1};}
    ...
    const backtest=dates.map((date,i)=>{
      const ss=Object.entries(dateMap[date])
        .map(([sector,stocks])=>{
          ...
          return{sector,leader:sorted[0],avgChange:stocks.reduce((s,x)=>s+x.change,0)/stocks.length};
        })
      ...
      return{date,topSector:top?.sector||null,leader:top?.leader?.name||null,leaderChange:top?.leader?.change||null,nextDayReturn,
        topSectors:ss.slice(0,5).map(s=>({sector:s.sector,avg:+s.avgChange.toFixed(2),leader:s.leader?.name,leaderChange:s.leader?.change}))};
    });
    const won=backtest.filter(d=>d.nextDayReturn>0).length;
    const tot=backtest.filter(d=>d.nextDayReturn!==null).length;
    res.status(200).json({ok:true,from,to,stockCount:Object.keys(results).filter(k=>results[k].prices).length,dates:dates.length,winRate:tot?Math.round(won/tot*100):0,backtest});
